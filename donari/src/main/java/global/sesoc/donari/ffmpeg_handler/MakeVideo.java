...
public class MakeVideo {
	...
	public void insertText(String inputPath, String outputPath) {
		String[] code = { ffmpegPath, "-i", inputPath, "-vf", 
		 "drawtext=fontfile=HMKMMAG.TTF:text='황부로짱...':fontcolor=white:fontsize=40:x=(w-tw)/2:y=(h/PHI)",
	...
	public void reformatImg(String inputPath, String outputPath, String width, String height) {
		String[] code = new String[] {
				ffmpegPath, "-i",
				inputPath, "-vf", "scale="+width+":"+height+":force_original_aspect_ratio=decrease,pad="+width+":"+height+":(ow-iw)/2:(oh-ih)/2", outputPath};
	...
	/**
	 * 만들어진 concat demuxer 텍스트파일을 이용해 모든 동영상을 합친다.
	 * 음악 없는 버전
	 */
	public void mergeVids(String inputPath, String outputPath) {

		String[] code = { ffmpegPath, "-y", "-f",
				"concat", "-safe", "0", "-i", inputPath, "-c", "copy", outputPath };
	...
	public void mergeVids(String inputPath, String outputPath, String musicPath) {
		...
		String[] code = { ffmpegPath, "-y", "-f",
				"concat", "-safe", "0", "-i", inputPath, "-i", musicPath,"-c", "copy", outputPath };
	...
	public void imgToVid(String inputImgPath, String outputVidPath, String durationSec) {
		String[] code = { ffmpegPath, "-r",
				"1/" + durationSec, "-i", inputImgPath, "-c:v", "libx264", "-r", "30", "-y", "-pix_fmt", "yuv420p",
				outputVidPath };
		executeCmd(code);
	}
	
	/**
	 * 영상에 Fade in 트렌지션을 삽입한다. startFrame은 시작프레임, durationFrame 재생시간(프레임)
	 * 1초 = 30프레임. (startFrame + durationFrame) / 30 이 동영상 재생시간을 넘지 않도록 할것 
	 */
	public void insertFadeIn(String inputVidPath, String outputVidPath, String startFrame, String durationFrame) {
		String[] code = { ffmpegPath, "-i",
				inputVidPath, "-y", "-vf", "fade=in:" + startFrame + ":" + durationFrame, outputVidPath };
		executeCmd(code);
	}

	/**
	 * 영상에 Fade out 트렌지션을 삽입한다. startFrame은 시작프레임, durationFrame 재생시간(프레임)
	 * 1초 = 30프레임. (startFrame + durationFrame) / 30 이 동영상 재생시간을 넘지 않도록 할것 
	 */
	public void insertFadeOut(String inputVidPath, String outputVidPath, String startFrame, String durationFrame) {
		String[] code = { ffmpegPath, "-i",
				inputVidPath, "-y", "-vf", "fade=out:" + startFrame + ":" + durationFrame, outputVidPath };
		executeCmd(code);
	}
	
	/**
	 * 떨림을 삽입한다.
	 * @param inputVidPath 인풋 경로
	 * @param outputVidPath 아웃풋 경로
	 */
	public void insertVibe(String inputVidPath, String outputVidPath, String level) {
		String[] code = new String[] { ffmpegPath,
				"-i", inputVidPath, "-vf", "crop=1200:700:(in_w-out_w)/2+((in_w-out_w)/2)*sin(ceil(n/4)):(in_h-out_h)/2 +((in_h-out_h)/2)*sin(1.6*ceil(n/4))",
				"-acodec", "copy", "-qscale", "0", outputVidPath };
		executeCmd(code);
	}
